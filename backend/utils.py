...
def scrape_product(html_content) -> dict[str, str]:
    ...
    try:
        ...
        product_image_url = soup.select_one('div.product-image-wrapper.product-wrapper-inline picture source')['srcset']
...
def add_or_update_sheet(product_data: dict) -> None:
    ...
    sht.append_row([product_data['name'], product_data['slug'], product_data['description'], float(product_data['price']) * 1500, 0, product_data['image_name'], 4.7])
